...
GET    /api/work-orders/upcoming # next 30 days
```
...
### 1. Dashboard
- KPI cards: assets, active strategies, WOs due this week
...
### 6. Work Orders
- Calendar + list view
- Filter by asset / site / status
- Create/update WO, log findings
...
- 4 KPI cards: Total Assets, Active Strategies, WOs Due This Week, WOs Overdue
  (each fetched from the appropriate API endpoint)
- A table of upcoming work orders (next 30 days) from GET /api/work-orders/upcoming
  with columns: WO#, Asset, Strategy, Scheduled Date, Status
...
- `sap_notification` on work_order table (already in schema)
- A sync service: GET SAP equipment list via OData API → upsert assets
- A write-back service: POST completed work orders → create SAP PM notifications
